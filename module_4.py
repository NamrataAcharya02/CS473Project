...
def call(mainDict, clusters):
    ...
    # Matrix : cols = tf score of words across all docs; row = docs

    if clusters == 0:
        # Try all silhouette scores (2...N) and find the maximum # of clusters
        silhouette_scores = np.zeros(len(mainDict.keys())+1)
        for i in range(2, len(mainDict.keys())):
            K = KMeans(n_clusters=i, n_init=100)
            label = K.fit_predict(all_words_mat)
            silhouette_scores[i] = silhouette_score(all_words_mat, label)
        clusters = silhouette_scores.argmax()
